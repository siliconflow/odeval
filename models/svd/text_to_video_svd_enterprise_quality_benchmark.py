...
def save_last_frame_as_image(output_frames, image_path, style, idx):
    """
    Save the last frame of the generated video frames as an image.
    """
    last_frame = output_frames[-1]
    image_file_path = os.path.join(image_path, style, f"{idx:05d}.jpg")
...
def evaluate_last_frame(image_path):
    ...
    """
    evaluation_result = hpsv2.evaluate(image_path)
    ...
    for height, width in resolutions:
        ...
        for style, prompts in all_prompts.items():
            for idx, prompt in enumerate(prompts):
                ...
                kwarg_inputs["image"] = generated_image
                print("Generating video...")
                output_frames = pipe(**kwarg_inputs).frames
                save_last_frame_as_image(output_frames[0], args.image_path, style, idx)
        end = time.time()
        ...
        evaluate_last_frame(args.image_path)
